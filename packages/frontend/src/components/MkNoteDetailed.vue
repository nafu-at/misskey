	...
	<article :class="$style.note" @contextmenu.stop="onContextmenu">
		...
		<div :class="$style.noteContent">
			...
			<div v-show="appearNote.cw == null || showContent">
				...
				<div v-if="appearNote.files && appearNote.files.length > 0">
					<MkMediaListDetailed ref="galleryEl" :mediaList="appearNote.files" :user="appearNote.user"/>
				</div>
		...
		<footer>
			...
			>
				...
				<p v-if="prefer.s.showRenotesCount && appearNote.renoteCount > 0" :class="$style.noteFooterButtonCount">{{ number(appearNote.renoteCount) }}</p>
			</button>
